...
SINKS = {
    ...
    "recv","read","send","write","system","popen",
    "execl","execlp","execle","execv","execvp","execve"
}
WORD = r'[A-Za-z_]\w*'
STR_LIT = r'"[^"\\]*(?:\\.[^"\\]*)*"'

# 공백 0회 이상 후 문자열 리터럴을 허용하도록 수정(\s*{STR_LIT})
...
def vuln_heuristic(code: str) -> int | None:
    ...
    - 싱크가 있고 가드가 없으면 → 1
    - 싱크가 있고 가드가 보이면 → 0
    - 싱크가 없으면 → None (보류: 원라벨 유지 또는 -1 처리)
    """
    if not code:
        return None
    has = contains_sink(code)
    if not has:
        return None  # 싱크 없음: 보류
...
def main():
    ...
    with inp.open("r", encoding="utf-8") as inf, out.open("w", encoding="utf-8") as outf:
        for i, line in enumerate(inf, 1):
            ...
            # 재라벨링 조건: 원라벨이 없거나(-1 포함) 또는 --force
            if orig_label is None or (isinstance(orig_label, int) and orig_label == -1) or args.force:
                # (a) 가젯 우선 판정
                ...
                # (c) 최종 보류 처리: 원라벨 보존 또는 -1
                if label is None:
                    if orig_label in (0, 1):
                        label = orig_label   # 보류 시 원라벨 보존
                    else:
                        label = -1
