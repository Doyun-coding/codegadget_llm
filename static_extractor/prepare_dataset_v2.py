#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# prepare_dataset.py — 텍스트 중복 제거, 그룹 홀드아웃 분할, split 후 과다 클래스 다운샘플

"""
python prepare_dataset.py \
  ./data/static/juliet_gadgets_normalized.jsonl \
  --outdir ./data/static/juliet_codebert_dataset \
...
def template_key(file_path: str) -> str:
    # 템플릿 홀드아웃을 좀 더 공격적으로: 상위 폴더/파일명 기준
    p = Path(file_path or "")
    name = p.stem
...
def group_split(records, groups, labels, seed, test_size, val_size):
    ...
    v_rel, t_rel = next(gss2.split(hold_records, hold_labels, hold_groups))
    val_idx  = [hold_idx[i] for i in v_rel]
    test_idx = [hold_idx[i] for i in t_rel]
    return train_idx, val_idx, test_idx

def rebalance_split(records, idxs, target=0.5, tol=0.1, seed=42):
    """과다 클래스 다운샘플 (split 내부에서만)."""
